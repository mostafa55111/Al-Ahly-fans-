...
class FirebaseRemoteDataSource {
  /// Firestore shared collections across club apps.
  static const String reelsCollection = SharedFirebaseCollections.reels;
  static const String productsCollection = SharedFirebaseCollections.products;
  ...
  Future<firestore.DocumentSnapshot> getDocument(
  ...
  Stream<firestore.DocumentSnapshot> getDocumentStream(
      String collection, String docId) {
    return _firestore.collection(collection).doc(docId).snapshots();
  }

  Stream<firestore.QuerySnapshot> getCollectionStream(
    String collection, {
    firestore.Query? query,
    int? limit,
    String? orderBy,
  ...
  Future<String> uploadFile(
      String path, Uint8List fileData, String contentType) async {
    final ref = _storage.ref().child(path);
    final uploadTask =
        await ref.putData(fileData, SettableMetadata(contentType: contentType));
    return await uploadTask.ref.getDownloadURL();
  }

  Future<String> uploadFileFromPath(
      String path, File file, String contentType) async {
    final ref = _storage.ref().child(path);
    final uploadTask =
