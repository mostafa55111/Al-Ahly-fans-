...
class _FanSquadBuilderState extends State<FanSquadBuilder>
  ...
  final Map<String, int> _shakeTicks = <String, int>{};
  ...
  void _fireVote(PastPlayerDto p) {
    final existing = widget.myVotedPlayerId;
    if (existing != null && existing.isNotEmpty) {
      if (existing != p.id) {
        HapticFeedback.heavyImpact();
        setState(() {
          _shakeTicks[p.id] = (_shakeTicks[p.id] ?? 0) + 1;
        });
        return;
      }
                                  ...
                                  children: [
                                    ...
                                    ...List.generate(_board.slotCount, (index) {
                                      ...
                                      Widget card = FifaCardWidget(
                                        ...
                                        shakeTrigger: widget.votingMode ? (_shakeTicks[p.id] ?? 0) : 0,
            ...
            PositionedDirectional(
              ...
              child: RepaintBoundary(
                child: _FloatingGlassSquadPanel(
                  ...
                  voteShakeTicks: _shakeTicks,
                  votePulseTicks: _pulseTicks,
...
class _FloatingGlassSquadPanel extends StatefulWidget {
  const _FloatingGlassSquadPanel({
    ...
    this.onVote,
    this.myVotedPlayerId,
    required this.voteShakeTicks,
    required this.votePulseTicks,
  });
  ...
  final ValueChanged<PastPlayerDto>? onVote;
  final String? myVotedPlayerId;
  final Map<String, int> voteShakeTicks;
  final Map<String, int> votePulseTicks;
                        ...
                        child: TabBarView(
                          ...
                          children: [
                            _BenchTab(
                              ...
                              myVotedPlayerId: widget.myVotedPlayerId,
                              voteShakeTicks: widget.voteShakeTicks,
                              votePulseTicks: widget.votePulseTicks,
                            ),
                            _ReservesTab(
                              reserves: reserves,
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        Widget card = FifaCardWidget(
          ...
          pulseTrigger: votingMode ? (votePulseTicks[p.id] ?? 0) : 0,
          shakeTrigger: votingMode ? (voteShakeTicks[p.id] ?? 0) : 0,
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    Widget card = FifaCardWidget(
                      ...
                      isVotingMode: widget.votingMode,
                      pulseTrigger:
                          widget.votingMode ? (widget.votePulseTicks[p.id] ?? 0) : 0,
                      shakeTrigger:
                          widget.votingMode ? (widget.voteShakeTicks[p.id] ?? 0) : 0,
