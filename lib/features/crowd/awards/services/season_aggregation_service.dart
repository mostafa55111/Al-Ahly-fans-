...
/// تجميع نسر/فارس الموسم من مجموع أصوات كل جلسات الموسم.
class SeasonAggregationService {
  SeasonAggregationService(this._awards);
  ...
  Future<PeriodWinnerAward?> computeAndPersist({
    ...
    final year = int.tryParse(seasonKey);
    ...
    final inSeason =
        matches.where((m) => m.seasonKey == seasonKey).toList(growable: false);
    if (inSeason.isEmpty) return null;

    final winner = _aggregateFromMatches(inSeason, closedAtServerMs);
    if (winner == null) return null;

    await _awards.setSeason(
      clubTag: clubTag,
      seasonKey: seasonKey,
      award: winner,
    );
