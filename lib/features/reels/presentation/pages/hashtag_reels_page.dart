...
/// قائمة ريلز بنفس الهاشتاج — استعلام Firestore `hashtags array-contains`.
///
/// قد يتطلّب فهرساً مركّباً في Firebase (hashtags + app_source + timestamp).
class HashtagReelsPage extends StatefulWidget {
  const HashtagReelsPage({
    super.key,
    required this.hashtag,
    required this.accentColor,
  });

  /// بدون رمز #، مُطبَّع عادةً بـ [ReelsHashtagUtils.normalizeTag].
  final String hashtag;
  final Color accentColor;

  @override
  State<HashtagReelsPage> createState() => _HashtagReelsPageState();
}

class _HashtagReelsPageState extends State<HashtagReelsPage> {
  ...
  Future<void> _load() async {
    try {
      final list =
          await context.read<ReelsFeedCubit>().fetchReelsForHashtag(widget.hashtag);
      if (!mounted) return;
      setState(() {
        _reels = list;
        _loading = false;
        _error = list.isEmpty ? 'لا توجد ريلز بهذا الهاشتاج بعد' : null;
      });
      ...
      setState(() {
        _loading = false;
        _error =
            'تعذّر تحميل الهاشتاج — تحقق من فهارس Firestore أو الاتصال\n$e';
      });
    }
  }
  ...
  Future<void> _shareVideo(VideoModel reel) async {
    final accent = AppConfig.reelsFirestoreClubTag == 'ahly'
        ? AppColors.royalRed
  ...
  /// مزامنة صف مع الكيوبت؛ لو الريل خارج الفيد الرئيسي نحدّث محلياً بنفس منطق التبديل.
  ...
  /// بعد double-tap like فقط — لا نعكس الحالة لو كان معجباً مسبقاً.
  ...
  Widget build(BuildContext context) {
    final tagLabel =
        widget.hashtag.startsWith('#') ? widget.hashtag : '#${widget.hashtag}';
      ...
      appBar: AppBar(
        ...
        title: Text(
          ...
          style: TextStyle(
            ...
            shadows: [
              Shadow(color: widget.accentColor.withValues(alpha: 0.4), blurRadius: 10),
            ],
          ),
        ),
      ),
